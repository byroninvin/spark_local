...
object GroupFavTeacher4 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sbPatitioner = new SubjectPartitioner2(subjects)
    //和一组合在一起(这种方式不好,调用了两次map方法
    //    val map: RDD[((String, String), Int)] = subjectAndteacher.map((_, 1))

    //聚合时就按照就按照指定的分区器进行分区
    //该RDD一个分区内仅有一个学科的数据
    val reduced: RDD[((String, String), Int)] = subjectAndteacher.reduceByKey(sbPatitioner,_ + _)
...
//    println(result.toBuffer)
    //
    sorted.saveAsTextFile("F:\\projectJ\\aaa")
