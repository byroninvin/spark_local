...
//      .select("did","vid","exploded", "rating1").show(false)
    ...
    spark.sql(
      """
        |select * from recommendation.user_play_history_animation_new where day='2019-09-08' limit 1
      """.stripMargin)
        .select(explode(col("plays"))).toDF("key", "values")
        .show()
