package dmp.test.report


import dmp.test.beans.Log
import dmp.test.utils.RptUtils
import org.apache.commons.lang.StringUtils
import org.apache.spark.{SparkConf, SparkContext}

/**
  * Created by Joe.Kwan on 2018/9/3. 
  */
class AppAnalyseRpt extends App {

  if (args.length != 3) {
    println(
      """
        |dmp.test.report.AppAnalyseRpt
        |参数:
        | 输入路径
        | 字典文件路径
        | 输出路径
  ...
  sc.textFile(inputPath)
    ...
    .map(log => {
      ...
      val rtb = RptUtils.caculateRtb(log.iseffective, log.isbilling,
        log.isbid, log.adorderid, log.iswin, log.winprice, log.adpayment)
