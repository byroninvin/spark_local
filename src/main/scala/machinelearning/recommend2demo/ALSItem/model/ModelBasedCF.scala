package machinelearning.recommend2demo.ALSItem.model

import machinelearning.recommend2demo.ALSItem.util.BlasSim
...
//    val testDataRrame = getResource(spark, testHql).repartition(100)
    ...
    val trainAndTest = lines.map(line => {
      val fields = line.split("::")
      (fields(0), fields(1), fields(2).toDouble)
    }).toDF().randomSplit(Array(0.8, 0.2))
...
//    val trainAndTest = input.randomSplit(Array(0.8, 0.2))
//    val trainDataFrame = trainAndTest(0).cache()
//    val testDataRrame = trainAndTest(1).cache()
//
...
//    }).cache()
    ...
    // 存储模型
//    model.write.overwrite.save("F:\\projectJ\\data\\model_als_20181230")

    val itemFeature: Array[(Int, mutable.WrappedArray[Float])] = model.itemFactors.rdd.map(
      row => {
    ...
    val numItems: Int = itemIndex.count().toInt
    ...
    val item_sim_rdd: RDD[(String, List[(String, Double)])] = itemString.map(x => {
      (x._2, blasSim.getCosinSimilarity((itemFeature(x._1)._2.toVector), 5, None).toList)
    })

    item_sim_rdd.collect.foreach(println)
